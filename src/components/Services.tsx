...
function formatPrice(amountUsd: number, currency: "USD" | "NGN" = "USD"): string {
  if (currency === "NGN") {
    return `₦${(amountUsd * USD_TO_NGN_RATE).toLocaleString()}`;
  }
  return `$${amountUsd.toLocaleString()}`;
}

const getFormattedPriceEstimate = (estimate: string, currency: "USD" | "NGN" = "USD") => {
  if (!estimate) return "";
  if (currency === "NGN") {
    if (estimate.includes("$4,500")) {
      return `From ₦${(4500 * USD_TO_NGN_RATE).toLocaleString()}`;
    }
    if (estimate.includes("$12,000")) {
      return `From ₦${(12000 * USD_TO_NGN_RATE).toLocaleString()}`;
    }
  }
  return estimate;
};
...
export default function Services({ tiers, onRequestTier, pricingRates, currency = "USD" }: ServicesProps) {
  ...
  const formattedInvestment = formatPrice(grandTotal, currency);
              ...
              <div className="pt-8 border-t border-neutral-900/60">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  ...
                  >
                    ...
                    <span className={`text-[10px] ${activeProduct.isBespokeModel ? "text-neutral-700" : "text-neutral-500"} font-light block leading-relaxed`}>
                      Fine-tune features, age, size, ethnicity, and styling elements tailored to your precise collection brief (Curation included; +{formatPrice(pricingRates?.extraModelPrice ?? 150, currency)} per additional model variant).
                    </span>
                  </button>
                </div>
                ...
                <div className="space-y-6 pt-6 border-t border-neutral-900/40">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 border border-neutral-900 bg-neutral-950/20">
                        <div>
                          ...
                          <p className="text-[10px] text-neutral-400 font-light mt-1">
                            How many unique model castings are required? (First curation included, +{formatPrice(pricingRates?.extraModelPrice ?? 150, currency)} per additional variant)
                          </p>
                        </div>
                  ...
                  <div className="flex items-center justify-between border-b border-neutral-900 pb-2">
                    ...
                    <span className="text-[9px] text-neutral-500 italic">
                      {activeProduct.isBespokeModel && activeProduct.activeModelIndex > 0 ? `+${formatPrice(pricingRates?.extraModelPrice ?? 150, currency)} variant surcharge` : "No additional cost"}
                    </span>
                  </div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span className={`text-[10px] ${activeProduct.productionScope === "multi" ? "text-neutral-700" : "text-neutral-500"} font-light block leading-relaxed`}>
                        Highly dynamic. Showcases your collection across diverse curated settings, studio arrangements, or landscapes (+ {formatPrice(pricingRates?.extraLocationPrice ?? 200, currency)}).
                      </span>
                    </div>
                  </button>
                </div>
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-[9px] font-mono text-neutral-500 uppercase">
                              +{formatPrice(pricingRates?.videoPrice ?? 250, currency)} per cinematic short
                            </span>
                          </div>
